...
import { getFinanceDetail, passFinance, rejectFinance } from '@/api/finance'
...
async function fetchDetail() {
  const res: any = await getFinanceDetail(expenseId)
  detail.value = res.data
}

async function handlePass() {
  try {
    await ElMessageBox.confirm('确定审核通过吗？', '确认审核', { type: 'success' })
  } catch {
    return
  }
  submitting.value = true
  try {
    await passFinance(expenseId, comment.value)
...
async function handleReject() {
  ...
  submitting.value = true
  try {
    await rejectFinance(expenseId, comment.value)
...
<template>
  <div v-if="detail" style="max-width: 900px">
    ...
    <!-- Action (only for PENDING_FINANCE status) -->
    <el-card v-if="detail.status === 3">
      <template #header><span>财务审核</span></template>
      <el-input
        v-model="comment"
        type="textarea"
        :rows="3"
      ...
      <div style="margin-top: 16px; text-align: center">
        <el-button type="success" :loading="submitting" @click="handlePass">
          通 过
        </el-button>
        <el-button type="danger" :loading="submitting" style="margin-left: 24px" @click="handleReject">
          驳 回
        </el-button>
      </div>
    </el-card>
